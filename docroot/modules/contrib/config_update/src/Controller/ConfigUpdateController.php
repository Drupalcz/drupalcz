...
/**
 * @file
 * Contains \Drupal\config_update\Controller\ConfigUpdateController.
 */

namespace Drupal\config_update\Controller;
...
class ConfigUpdateController extends ControllerBase {
  ...
  public function import($config_type, $config_name) {
    ...
    return $this->redirect('config_update.report');
  }
  ...
  public function diff($config_type, $config_name) {
    ...
    $url = new Url('config_update.report');
  ...
  protected function generateReportLinks() {
    ...
    foreach ($definitions as $entity_type => $definition) {
      $links['type_' . $entity_type] = array(
        ...
        'url' => Url::fromRoute('config_update.report', array('report_type' => 'type', 'name' => $entity_type)),
      );
    }
    ...
    $links = array(
      'type_all' => array(
        ...
        'url' => Url::fromRoute('config_update.report', array('report_type' => 'type', 'name' => 'system.all')),
      ),
      'type_system.simple' => array(
        'title' => $this->t('Simple configuration'),
        'url' => Url::fromRoute('config_update.report', array('report_type' => 'type', 'name' => 'system.simple')),
      ),
    ) + $links;
    ...
    foreach ($modules as $machine_name => $module) {
      if ($machine_name != $profile) {
        $links['module_' . $machine_name] = array(
          ...
          'url' => Url::fromRoute('config_update.report', array('report_type' => 'module', 'name' => $machine_name)),
        );
      }
    }
    uasort($links, array($this, 'sortLinks'));
    ...
    foreach ($themes as $machine_name => $theme) {
      $links['theme_' . $machine_name] = array(
        ...
        'url' => Url::fromRoute('config_update.report', array('report_type' => 'theme', 'name' => $machine_name)),
      );
    }
    uasort($links, array($this, 'sortLinks'));
    ...
    $links['profile_' . $profile] = array(
      ...
      'url' => Url::fromRoute('config_update.report', array('report_type' => 'profile')),
    );
    $build['links']['#rows'][] = array(
  ...
  protected function generateReport($report_type, $value) {
    ...
    switch($report_type) {
      ...
      case 'theme':
        ...
        // Note that unlike ModuleHander::getName(), the ThemeHandler function
        // runs the theme label through checkPlain, so we should not escape it
        // again.
  ...
  protected function makeReportTable($names, $storage, $actions) {
    ...
    foreach ($names as $name) {
      ...
      $routes = array(
        ...
        'import' => 'config_update.import',
        'diff' => 'config_update.diff',
        'revert' => 'config_update.revert',
      );
      $titles = array(
